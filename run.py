...
def train_eval_network(dataset_name, train_gen, validate_gen, test_x, test_y, seq_len, epochs, batch_size,
    ...
    model_names = ('checkpoints/' +cnn_arch.__name__+
               '_weights.{epoch:02d}-{val_loss:.4f}.hdf5')
...
def evaluate(dataset_name, train_gen, validate_gen, test_x, test_y, seq_len, epochs, batch_size,
                       batch_epoch_ratio, initial_weights, size, cnn_arch, learning_rate,
                       optimizer, cnn_train_type, pre_weights, lstm_conf, len_train, len_valid, dropout, classes,
                       patience_es=15, patience_lr=5):
    """the function build, compine fit and evaluate a certain architechtures on a dataset"""
    ...
    model = BuildModel_basic.build(size=size, seq_len=seq_len, learning_rate=learning_rate,
                                   ...
                                   cnn_class=cnn_arch, pre_weights=pre_weights, lstm_conf=lstm_conf,
                                   cnn_train_type=cnn_train_type, dropout=dropout, classes=classes)
    res=model.evaluate_generator(validate_gen, steps=int(float(len_valid) / float(batch_size)), max_queue_size=10, workers=1, use_multiprocessing=False, verbose=0)
...
root_dir='/home/eric/data/violence_recognition'
# static parameter for the netwotk
datasets_videos = dict(
    hocky=dict(hocky=os.path.join(root_dir,"HockeyFights")),
    violentflow=dict(violentflow=os.path.join(root_dir,"violentflow")),
...
dataset_name='violentflow'

# crop_dark['hocky']=(11, 38)
...
cnns_arch = dict(ResNet50=ResNet50, InceptionV3=InceptionV3, InceptionResNetV2=InceptionResNetV2)  #
learning_rates = [1e-4, 1e-3]
use_augs = [True, False, ]
fix_lens = [20, 10]
optimizers = [(Adam,{})]
dropouts = [0.5]
cnn_train_types = ['retrain', 'static','evaluate']
...
if apply_hyper:
    ...
    hyper, results = hyper_tune_network(dataset_name=dataset_name, epochs=30,
    ...
    results = []
    # ResNet50 MobileNetV3_Large EfficientNetB0 ShuffleNetV2
    # RMSprop Adam

    cnn_arch, learning_rate, optimizer, cnn_train_type, dropout, use_aug, fix_len = EfficientNetB1, 0.0001, (
    Adam, {}), cnn_train_types[0], 0.0, True, 20
...
if(cnn_train_type=='retrain'):
    ...
    for dataset_name, dataset_videos in datasets_videos.items():
        train_gen, validate_gen, test_x, test_y, seq_len, len_train, len_valid = get_generators(dataset_name,
                                                                                                ...
                                                                                                use_crop=True,
                                                                                                crop_dark=crop_dark)
        result = train_eval_network(epochs=35, dataset_name=dataset_name, train_gen=train_gen, validate_gen=validate_gen,
    ...
    for dataset_name, dataset_videos in datasets_videos.items():
        ...
        train_gen, validate_gen, test_x, test_y, seq_len, len_train, len_valid = get_generators(dataset_name,
                                                                                                    ...
                                                                                                    use_crop=True,
                                                                                                    crop_dark=crop_dark)
        evaluate(epochs=35, dataset_name=dataset_name, train_gen=train_gen, validate_gen=validate_gen,
                                        test_x=test_x, test_y=test_y, seq_len=seq_len, batch_size=batch_size,
                                        batch_epoch_ratio=0.5, initial_weights=initial_weights, size=figure_size,
                                        cnn_arch=cnn_arch, learning_rate=learning_rate,
                                        optimizer=optimizer, cnn_train_type=cnn_train_type,
